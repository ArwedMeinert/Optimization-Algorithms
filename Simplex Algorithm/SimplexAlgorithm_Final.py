...
class SimplexTable(SimplexDataComposite):
    ...
    def solveSimplex(self,debug=False):
        ...
        # Find the pivot column (the column with the most positive coefficient in the objective function)
            ...
            minValue = 0 # needs to be negative
            pivotColumn = -1
            # Prioritize real variables over slack variables for pivoting
            for col in range(len(self.children[1].data)-self.slackVariables):  # Check real variables first
                if self.objectiveFunction[col] < minValue and col not in pivotedColumns:
                    minValue = self.objectiveFunction[col]
                    pivotColumn = col
        
            # If no real variable is chosen, check slack variables
            if pivotColumn == -1:
                for col in range(len(self.children[1].data)-self.slackVariables, len(self.objectiveFunction) - 1):
                    if self.objectiveFunction[col] < minValue and col not in pivotedColumns:
                        minValue = self.objectiveFunction[col]
                        pivotColumn = col
            ...
            if pivotColumn == -1:
                break
            pivotElement = self.children[pivotRow].data[pivotColumn]
            RowCopy=self.children[pivotRow].data[:]
            RowNew=[]
            for i in range(len(RowCopy)):
                RowNew.append(RowCopy[i]/pivotElement)
            ...
            if debug: self.display()
            pivotedColumns.append(pivotColumn)
            SimplexIteration+=1
            nextIteration = any(element < 0 for element in self.objectiveFunction[:-1])
...
print("Ex.4 (Page 29, Lecture 2)")
#Ex.4 (Page 29, Lecture 2)
objective=SimplexObjective(name='objective',inputEquation=[1,1,2],maximise=True)
eq1=SimlexEquation(name='eq1',inputEquation=[2,1,1,50],smaller=True)
eq2=SimlexEquation(name='eq2',inputEquation=[2,1,0,36],smaller=False)
eq3=SimlexEquation(name='eq3',inputEquation=[1,0,1,10],smaller=False)
...
for i in range(3):
    print()
print("Ex.5 (Page 32, Lecture 2)")

#Ex.5 (Page 32, Lecture 2)
